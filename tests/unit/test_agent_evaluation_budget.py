...
def test_live_budget_persists_and_caps_concurrent_requests(
    ...
    monkeypatch.setattr(settings, "agent_evaluation_budget_usd", Decimal("0.10"))

    def reserve(_: int) -> bool:
        return reserve_agent_evaluation("claude-opus-5-5", "system", [], [], 2000)

    with ThreadPoolExecutor(max_workers=8) as pool:
        results = list(pool.map(reserve, range(8)))
    assert sum(results) == 1
    assert Decimal(json.loads(ledger.read_text())["reserved_usd"]) <= Decimal("0.10")
...
def test_corrupt_or_unknown_budget_fails_closed(
    ...
    assert not reserve_agent_evaluation("claude-opus-5-5", "", [], [], 100)
