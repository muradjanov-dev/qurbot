...
class CanonicalProduct(Base, TimestampMixin):
    ...
    def display_image_url(self) -> str:
        """Return an uploaded local photo or a product-family image on our server."""
        if (
            self.image_url
            and self.image_url.startswith("/")
            and not self.image_url.startswith("//")
        ):
            return self.image_url
        if isinstance(self.attributes, dict):
            legacy = self.attributes.get("image_url")
            if isinstance(legacy, str) and legacy.startswith("/") and not legacy.startswith("//"):
                return legacy
        filename = photo_filename(self.category.slug if self.category else "", self.name_uz)
